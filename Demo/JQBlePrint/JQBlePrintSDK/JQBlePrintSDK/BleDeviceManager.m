...
-(void)writeCharacteristic:(CBPeripheral *)peripheral
    ...
    if(characteristic.properties & CBCharacteristicPropertyWriteWithoutResponse || characteristic.properties & CBCharacteristicPropertyWrite){
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            for (int i = 0; i < [value length]; i += BLE_SEND_MAX_LEN) {
                // 预加 最大包长度，如果依然小于总数据长度，可以取最大包数据大小
                if ((i + BLE_SEND_MAX_LEN) < [value length]) {
                    NSString *rangeStr = [NSString stringWithFormat:@"%i,%i", i, BLE_SEND_MAX_LEN];
                    NSData *subData = [value subdataWithRange:NSRangeFromString(rangeStr)];
                    NSLog(@"%@",subData);
//                    [peripheral writeValue:subData forCharacteristic:characteristic type:_writeStruct];
                    
                    [self write:peripheral withData:subData];
                    ...
                    NSString *rangeStr = [NSString stringWithFormat:@"%i,%i", i, (int)([value length] - i)];
                    NSData *subData = [value subdataWithRange:NSRangeFromString(rangeStr)];
//                    [peripheral writeValue:subData forCharacteristic:characteristic type:_writeStruct];
                    [self write:peripheral withData:subData];
...
//    [self writeCharacteristic:peripheral characteristic:self.characteristic value:data];
//    return;
    
    NSInteger packgeSize = 20;
    
    if(data.length > packgeSize) {
        NSInteger i = 0;
        while ((i + 1) * packgeSize <= data.length) {
//            [NSThread sleepForTimeInterval:0.012];
            NSData *dataSend = [data subdataWithRange:NSMakeRange(i * packgeSize, packgeSize)];
